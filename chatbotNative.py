# -*- coding: utf-8 -*-
"""
Created on Fri Apr 14 11:53:36 2023
...
#lstm_dropout=tf.keras.layers.Dropout(1-keep_prob)(encoder_outputs)
...
batch_size = 20
n_batches = len(sorted_clean_questions) // batch_size


# =============================================================================
# for i in range(len(sorted_clean_answers[start_index:end_index])):
#     sorted_clean_answers[start_index:end_index][i] = sorted_clean_answers[start_index:end_index][i][1:]
# decoded_out = pad_sequences(sorted_clean_answers[start_index:end_index], maxlen=maxlen_answers, padding='post')
...
for epoch in range(100):
    for batch in range(n_batches):
        ...
        encoder_input_data = np.array(pad_sequences(sorted_clean_questions[start_index:end_index], 
                                                     maxlen=maxlen_questions, padding='post'))
        decoder_input_data = np.array(pad_sequences(sorted_clean_answers[start_index:end_index], 
                                                     maxlen=maxlen_answers, padding='post'))
        
        nsorted=sorted_clean_answers[start_index:end_index]
        for i in range(len(nsorted)):
            nsorted[i] = nsorted[i][1:]
        decoded_out = pad_sequences(nsorted, maxlen=maxlen_answers, padding='post')
        onehot_answers = utils.to_categorical(decoded_out, VOCAB_SIZE)
        decoder_output_data = np.array(onehot_answers)

        loss = model.train_on_batch([encoder_input_data, decoder_input_data], decoder_output_data)
...
while prepro1 != 'q':
    ...
    try:
        ...
        print("sorry didn't got you , please type again :( ", + Exception)
